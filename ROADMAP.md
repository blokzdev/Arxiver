...
## Phase v2 — Richer experience (mapped 2026-06-14; subphases planned individually)

Dependency-ordered. The standout pillar is **AI understanding** (multi-provider BYOK + on-device/edge inference). Build order: **v2.1 → v2.2**, then v2.3/v2.4/v2.5. Each subphase gets its own Plan-tool session + green-build PR before building; this section is the phase-level map. Architecture for the AI platform lives in **`docs/SPEC-AI-PROVIDERS.md`**.

- **v2.1 — AI Provider Platform** (new `:core:ai` module): provider-agnostic `AiProvider` (streaming `Flow`), multi-key vault (generalize `TokenVault` → provider→key), device-capability tiering with graceful degradation. **Ships in two sub-phases:**
  - **v2.1a — cloud BYOK first:** `AiProvider` abstraction + provider-neutral message/response types; **Claude** (Anthropic Messages API, `x-api-key`, SSE) + **Gemini** (Gemini Developer API, `streamGenerateContent`) transports over OkHttp — *not* Firebase AI Logic (that proxies a developer's key, not the end-user's); Settings "connect a provider" key entry (mirrors `RoutineSetupScreen`'s password field) + provider selection; "what leaves the device" preview (reuse `DispatchSheet` + `PayloadBuilder` structural redaction).
  - **v2.1b — on-device next:** `DeviceCapability`/`InferenceTier` diagnostics (RAM via `ActivityManager`, AICore availability); **Gemma 4 E2B** (0.84 GB text-only, Apache-2.0, `.litertlm`) via **LiteRT-LM** (`com.google.ai.edge.litertlm:litertlm-android`), downloaded with the `ModelDownloader` + unmetered `SyncScheduler` pattern; optional **system Gemini Nano** (ML Kit GenAI / AICore on flagships, no download); auto-pick best tier with manual override.
- **v2.2 — Chat-with-paper & knowledge base (RAG):** on-device retrieval (text chunking + chunk embeddings reusing `EmbeddingService` + `VectorIndex.topK` + `HybridFusion`) over a user-curated **knowledge base** (flagged papers' abstract+notes, later full text); chat UI (per-paper "Ask" sheet + per-KB chat) + summaries via the v2.1 provider; new chat-history table/DAO; only retrieved chunks + the question leave the device. Depends on v2.1. (Routine *result round-trip* remains out of scope — research-confirmed; see SPEC-CLAUDE-BRIDGE.)
- **v2.3 — Reader depth:** PDF highlight/annotation anchored to text + margin notes (DB migration); TTS read-aloud (`TextToSpeech`); full-text PDF extraction → FTS + chunk embeddings (feeds v2.2 RAG).
- **v2.4 — Smarter discovery:** on-device per-user recommender (logistic-regression / linear-SVM over embeddings, à la arxiv-sanity, trained on saved/dismissed); "more like this" (`VectorIndex`); author following; trending within follows.
- **v2.5 — Visual & home:** visual citation-graph canvas (replaces list Connections); home-screen widgets via **Glance**; reading queue + streaks; relevance-ranked Today.
...
| Date | Decision |
|---|---|
| 2026-06-14 | v2 mapped at phase level (Phase v2 + `docs/SPEC-AI-PROVIDERS.md`), subphases planned individually. Standout pillar = **AI understanding**: a provider-agnostic AI platform (`:core:ai`) with **multi-provider BYOK** (Claude Messages API + Gemini Developer API over OkHttp — *not* Firebase AI Logic, which proxies a dev key not the end-user's) and **on-device/edge** inference (Gemma 4 E2B 0.84 GB Apache-2.0 via LiteRT-LM; optional system Gemini Nano via ML Kit/AICore) with device-capability tiering + graceful degradation. Build order: v2.1 platform (cloud BYOK first, on-device next) → v2.2 chat/RAG → v2.3/2.4/2.5. RAG retrieval stays on-device (reuse bge embeddings + `VectorIndex`); only retrieved chunks + question reach a cloud provider. |
