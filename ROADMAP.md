...
- [ ] M.2 Error/empty/loading consistency: promote `ErrorState` to `ui/components/` (shared); add loading+error states to Onboarding and a proper error state to PaperDetail's notFound; swap ad-hoc empty states (FilteredPapersScreen, RoutinesScreen) to shared `EmptyState` + loading skeleton
- [ ] M.3 Compose previews + a11y: add missing light/dark `@Preview` pairs (FilteredPapersScreen, SearchScreen, RoutinesScreen, NewCollectionDialog, onboarding states); add semantics to genuinely icon-only controls (PaperListItem selection checkbox); ensure each StatusChip tone carries a non-color cue
- [ ] M.4 Core (non-UI) test coverage: `:core:common` (AppResult/AppError), `:core:database` migration-test harness + untested DAOs (Note/Tag/LibraryEntry/Citation), `:core:model` mapping/serialization, `:core:network` pagination + error-recovery gaps
- [ ] M.5 ViewModel test coverage + onboarding de-flake: unit tests for all 9 untested ViewModels (injected test dispatchers + fakes); isolate `OnboardingFlowTest` Robolectric state and remove `@Ignore` so the first-run flow rejoins CI
- [ ] M.6 Docs/process + v1.2.0 release prep: document the payload schema-version bump process in SPEC-CLAUDE-BRIDGE; set `versionName = "1.2.0"` (reuse unreleased versionCode 4); Decision log + memory update
- [ ] **CHECKPOINT M:** full `./gradlew build` green (lint flake gone across two runs), all new test suites pass, `OnboardingFlowTest` green in-suite, `:app:assembleRelease` APK still unobfuscated

## v2 candidate backlog (do not work on these)

Routine **result round-trip** (webhook inbox) · in-app Claude API chat-with-paper · full-text PDF indexing · visual graph canvas · Play Store · tablet layouts · per-user preference learning beyond centroids (SVM à la arxiv-sanity) · HTML paper rendering (ar5iv) · **re-enable R8 code/resource shrinking** behind audited keep rules for every reflection-based lib (Hilt, DataStore/protobuf, Room, Retrofit, ONNX) with an on-device smoke test before release (disabled in v1.1.2 after two obfuscation casualties)
