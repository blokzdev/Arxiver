...
- **P1 — AI Provider Platform** (new `:core:ai` module): provider-agnostic `AiProvider` (streaming `Flow`), multi-key vault (generalize `TokenVault` → provider→key), device-capability tiering with graceful degradation. **Ships in two subphases:**
  ...
  - **P1.2 — on-device next:** `DeviceCapability`/`InferenceTier` diagnostics (RAM via `ActivityManager`, ML Kit Nano availability); **Gemma 4 E2B** (text-only, Apache-2.0, `.litertlm`) via **LiteRT-LM** (`com.google.ai.edge.litertlm:litertlm-android`), downloaded with the `ModelDownloader` + unmetered `SyncScheduler` pattern; **system Gemini Nano** via **ML Kit GenAI Prompt API** (`com.google.mlkit:genai-prompt`, flagships, no download); auto-pick best tier with manual override. External-API reference captured in `docs/reference/on-device-ai.md`. Three PR-sized steps:
    ...
    - [ ] **P1.2b — Gemma 4 E2B downloadable provider (Tier 2) + Settings UX.** LiteRT-LM dep; Gemma `ModelSpec` + 2nd `ModelDownloader`; `OnDeviceEngine` interface + `GemmaEngine` (mutex + `Engine.initialize()` on a dispatcher, mirrors `EmbeddingService`); `OnDeviceProvider` (delegates to first ready engine) registered in `ProviderRegistry`; `AndroidDeviceCapabilityProbe` (app); unmetered download worker; Settings on-device card (RAM/tier, download/delete, default radio when ready). CI tests with a fake engine; real inference → `VERIFICATION.md`.
    - [ ] **P1.2c — system Gemini Nano (Tier 1).** `genai-prompt` dep; `NanoAvailability` (`checkStatus()`) + `NanoEngine` (`generateContentStream`); folded into `OnDeviceProvider` (Nano preferred over Gemma); Settings shows Nano status + optional download. Flagship-device-bound → `VERIFICATION.md`.
- **P2 — Chat-with-paper & knowledge base (RAG):** on-device retrieval (text chunking + chunk embeddings reusing `EmbeddingService` + `VectorIndex.topK` + `HybridFusion`) over a user-curated **knowledge base** (flagged papers' abstract+notes, later full text); chat UI (per-paper "Ask" sheet + per-KB chat) + summaries via the P1 provider; new chat-history table/DAO; only retrieved chunks + the question leave the device. Depends on P1. (Routine *result round-trip* remains out of scope — research-confirmed; see SPEC-CLAUDE-BRIDGE.)
