...
- **P1 — AI Provider Platform** (new `:core:ai` module): provider-agnostic `AiProvider` (streaming `Flow`), multi-key vault (generalize `TokenVault` → provider→key), device-capability tiering with graceful degradation. **Ships in two subphases:**
  ...
  - **P1.2 — on-device next:** `DeviceCapability`/`InferenceTier` diagnostics (RAM via `ActivityManager`, AICore availability); **Gemma 4 E2B** (0.84 GB text-only, Apache-2.0, `.litertlm`) via **LiteRT-LM** (`com.google.ai.edge.litertlm:litertlm-android`), downloaded with the `ModelDownloader` + unmetered `SyncScheduler` pattern; optional **system Gemini Nano** (ML Kit GenAI / AICore on flagships, no download); auto-pick best tier with manual override.
