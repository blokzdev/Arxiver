...
- [ ] **K3 Streaming render** — the `AskSheet` renders tokens incrementally as they arrive (no freeze-then-dump); the Stop button cancels mid-stream cleanly and leaves the partial answer. _(P2.3 — `AskViewModel` logic CI-tested; on-device incremental rendering is device-bound)_
