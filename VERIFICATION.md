...
- [!] **J3 Gemini test connection** — same with a real Gemini Developer API key. _v2.0.2 (emu 2026-06-23): the user-supplied Gemini key (`AQ.…` shape) → **"Couldn't reach the provider"** (`ConnectionTest.Error`), not "Offline" — so the request reached Gemini and came back non-2xx/non-401/403. Two findings: (1) the key is the wrong **type** (Gemini Developer API keys are `AIza…`; `AQ.…` looks like an OAuth/STS token), so the happy path is unverified — **needs a standard `AIza…` key**; (2) candidate UX bug — `GeminiProvider.toAppError` maps a 4xx that isn't 401/403 to `Upstream(code)`, but the failure surfaced as the generic "Couldn't reach the provider" rather than "Key rejected", so a bad Gemini key reads like a network outage. Tracked for a follow-up once a valid key confirms the exact status._ _(P1.1b)_
