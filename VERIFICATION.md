...
- [ ] **K3 Streaming render** — the `AskSheet` renders tokens incrementally as they arrive (no freeze-then-dump); the Stop button cancels mid-stream cleanly and leaves the partial answer. **Re-verify after F2:** per-paper Ask **and** collection chat return non-blank streamed answers on the S20 (the `-web` model caused blank bubbles + "Couldn't get an answer", 2026-06-19); a generation that yields no tokens now shows the error instead of a blank bubble. _(P2.3 / F2 — `AskViewModel` logic CI-tested; on-device generation device-bound)_
