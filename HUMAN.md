...
| OPEN | **Re-verify Qwen-only Ask on your real device (you hold the exact repro).** Update the app past the PA.6 hotfix, then on the device that showed "No AI provider is set up yet": open any paper → Ask → fire a preset. Expect the sheet to say "On-device — nothing leaves your device" and stream an answer (a 0.6B model — simpler prose than Gemma is correct). If your phone is low on storage you may now see a graceful "not enough free storage" error instead of a crash — that's the new guard working; free space and retry. Report back and I'll flip the `VERIFICATION.md` K20 row to a real-device entry. | `VERIFICATION.md` K20 |
