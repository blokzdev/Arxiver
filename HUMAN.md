...
| Status | Item | Detail |
|---|---|---|
| OPEN | **Device-verify the P-HTML reader after PH.4** (`VERIFICATION.md` §M). A live native 2026 paper **and** a pre-2023 ar5iv paper render end-to-end; **airplane-mode** render from cache with **zero egress**; **native MathML** measured with a stress sample (display eq + matrix + stretchy delimiters + scripts) on **Pixel_3a_API_34 AND an API-26/27 old-WebView AVD** — that verdict decides whether PH.8's bundled MathML fallback is needed; fallback-to-PDF reachable; TalkBack reads it. CI can't run a device; these never block `[x]` but must be observed before the reader is trusted. | `VERIFICATION.md` §M |
