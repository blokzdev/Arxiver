...
3. **Refine, maximize, and harvest:** fold quick wins + gap-closers *into* the plan and drop redundant/dead steps — strengthen it to its best form *within scope*. **Proactively choose the better design and take it** (Co-Founder standing directive, 2026-07-05): don't merely execute the plan faithfully — actively hunt for the materially superior design (clearer / safer / more-correct / more-idiomatic / better-UX) and adopt it. Architecture / correctness / testability / clarity wins are adopted directly; product/UX wins are surfaced to the Co-Founder via `HUMAN.md` and taken on the nod. Guard against gold-plating — "better" must be a genuine win, not scope creep. Then **harvest the out-of-scope upside**: opportunities the adversarial pass surfaces that serve the maximal product vision but don't belong in this PR. **Track every one — never silently drop a good idea** (a deferral is a *recorded decision*, not an omission). Route each to the fitting home, in the same commit as the work: a future **ROADMAP** phase/subphase (if it has a natural place), the ROADMAP **backlog** (if speculative/unscheduled), or a **`HUMAN.md`** carve-out (if it needs the Co-Founder's steer). Don't creep the current PR; do leave a breadcrumb so the vision compounds instead of leaking.
