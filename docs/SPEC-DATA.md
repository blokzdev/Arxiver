...
### inbox_items
`inbox_items(paper_id PK → papers, follow_id → follows, arrived_at, state TEXT /* new / seen / saved / dismissed */, score REAL? /* two-sided Rocchio relevance [0,1], P4 */)`. Saving creates a `library_entries` row and marks state=saved. Dismissed rows pruned after 30 days.
