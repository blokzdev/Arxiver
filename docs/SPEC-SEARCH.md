...
|---|---|---|---|
| **Local** | library + inbox + cached papers | FTS4 + vector fusion (below) + a full-text body leg | always, offline |
| **Online** | arXiv + other preprint sources | arXiv natively; other sources via OpenAlex / Semantic Scholar (one source per submit — see the P-Explorer PE.3 note) | network required |
...
- Field prefixes `ti:` `au:` `abs:` `cat:` and quoted phrases are parsed into a structured `Query`:
  - **Local:** mapped to FTS4 column filters (`title:`, `authors:`…) and category WHERE clauses; field-scoped queries skip the semantic leg unless free text remains (embedding a bare `cat:cs.LG` is meaningless).
...
```
parallel:
  keyword leg:  FTS4 MATCH, bm25 weights (title 10, abstract 5, authors 3, notes 2) → top 30
...
- If the embedding model isn't downloaded yet, local search degrades to pure FTS4 silently (plus a one-time hint to enable semantic search).
...
- Storage: L2-normalized float32[384] per paper as a Room BLOB (SPEC-DATA §4); vector KNN is a chunked cosine scan (`VectorIndex`), fast at our corpus scale. (`sqlite-vec` is a tracked v2 optimization, not the shipped path.)
...
- Vector: `VectorIndex` (BLOB chunked cosine scan) — correctness via `VectorMath` / dot-similarity tests + a scan perf/cost guard (`VectorScanPerfGuardTest`). A dual `sqlite-vec` contract test arrives only if/when the v2 backend lands.
