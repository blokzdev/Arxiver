...
- **Tier 1 — system Gemini Nano** (ML Kit GenAI APIs over **AICore**). Flagship-allowlisted hardware (Pixel 9, Galaxy S25, etc.); no download, OS-managed lifecycle, NPU-accelerated, free. Gate on the ML Kit availability/feature check. Best when present.
...
**Capability detection & degradation.** `DeviceCapability` reads total RAM (`ActivityManager.MemoryInfo.totalMem`) and AICore/ML-Kit GenAI availability → an `InferenceTier` recommendation. Default selection order: **Nano → Gemma 4 E2B → cloud BYOK → none**; the user picks a provider in Settings and the choice is remembered. If a tier becomes unavailable (model deleted, key cleared, device unsupported), fall back down the order with a clear message.
