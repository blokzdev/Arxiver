...
- **`search_semantic_scholar`** (PT.3, **SHIPPED**) — `{ query: string, limit?: int (≤25), venue?: string, year?: {from,to} }` → S2 `/graph/v1/paper/search`; per hit `{ paperId, title, abstractSnippet?, tldr?, year, venue, authors, citationCount, externalIds{arxiv?,doi?,pubmed?}, openAccessPdf?, importable }`. The **`importable`** flag = "the hit's `externalIds.ArXiv` parses" — it tells the model up front which hits `import_to_library` accepts (arXiv-keyed schema; non-arXiv hits are read-only — §10, §14). **MVP is search-only:** the S2 `paper/{id}` details, `paper/batch`, and recommendations endpoints are **deferred to the ROADMAP backlog** (SPEC §14; no MVP caller — import routes through the arXiv `getPaper`, not S2 details). Each is one client method + one `ToolSpec` row when demanded, no seam change.
