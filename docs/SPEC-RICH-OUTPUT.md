...
- **R4.2** — rendered **image/PDF** export (answer/diagram → PNG/PDF): needs a FileProvider + `EXTRA_STREAM`; the WebView capture needs a `createPrintDocumentAdapter()`→PDF spike (the async-Mermaid off-screen bitmap is flaky).
