...
- **STRUCTURED (Gemma E2B):** base prompt + a **`TABLE::` low-syntax nudge** (P-Atlas PA.4), with **no LaTeX / no diagrams / no GFM pipes**. Rather than ask a ~2B model for valid GFM table *syntax* (~40–85% reliable), the addendum teaches a block fenced by `TABLE::`/`::TABLE`, one row per line, cells split by `~|~`, `[n]` citations inline — a low-syntax format small models nail 0-shot (arXiv:2506.19512). On settle, the pure `StructuredTableTransform` (`:core:ai`) renders it to a **guaranteed-valid GFM table** (gate `parseConfidence ≥ 0.70`) or a **grounded bulleted list** (the ~1B "table reversal", arXiv:2412.17189), via a `settleStructured` helper gated on `PreparedChat.richness == STRUCTURED` at both the persist (`ChatRepository`) and display (`AskViewModel`) seams (so DB == UI; FULL/PLAIN/no-fence answers are byte-identical). Constrained decoding was rejected (unavailable from LiteRT-LM's Kotlin binding (#1662), and the wrong tool at our scale). Mermaid (~60% valid) + LaTeX (~9.7% error) stay cloud-only.
...
- **R3c** ✅ — **Ask preset library (text-only):** an extensible `AskPresets` registry of 9 one-tap tools (Summarize is member #1, reusing its prompt; Key contributions, Explain method, Limitations, Compare related, Explain simply, Key terms, BibTeX, Reproducibility). A preset is just a **predefined question** run through the existing grounded `ask()` — so the request is byte-identical to a typed question (**no assembler change, redaction goldens untouched**). A horizontally-scrollable `AssistChip` row above the input, scope-filtered (`forScope`) and disabled while streaming. The *task* ("what") dimension.
- **R3b** ✅ — **3-mode depth dial** (Quick / Standard / Max): `ChatMode` resolves to a directive **prepended to the user turn** in `ChatContextAssembler` (never the system prompt) — Standard = empty = byte-identical request (goldens untouched); Quick = terse; Max = detailed + a **cloud-only** table/Mermaid nudge. A `SingleChoiceSegmentedButtonRow` in the Ask sheet; applies to typed questions and R3c presets alike (the preset × mode grid). The *depth* ("how") dimension — **orthogonal** to presets. Rule: modes govern depth only; *audience* ("non-specialist") is a preset (ELI5), never a mode facet. **Follow-up chips (heuristic + model-gen-in-Max) → R3b.2.**
- **R3d** — **multimodal "with figures"** (vision, cloud-only): PDF page → downscaled base64 → vision request; provider-vision gating (on-device hidden), privacy-preview disclosure, payload limits. The *input* ("with what") axis; surfaces as a `requiresVision` preset. Phased: **R3d.1** ✅ (request model + provider serialization + `vision` capability — `ChatMessage.images`, Anthropic string-or-array content, Gemini `inlineData`; text-only byte-identical) → **R3d.2** ✅ (PDF page → downscaled JPEG base64 via an injected `PageImageSource`; network-free local-PDF glob, `eraseColor(WHITE)`, long-edge cap ≤1568px) → **R3d.3** ✅ (privacy disclosure + assembler/repo plumbing — optional `attachment: ChatImage?` through `prepare()`→`assemble()`, folded onto the final user turn **only when `capability.vision`**; `ChatPreviewBuilder` discloses the image — page/paper label, mime, approx KB — in **both** the human `text` and golden `json`, never the base64 bytes) → **R3d.4** ✅ ("Summarize with figures" preset — `AskPreset.requiresVision`, PAPER scope — gated at sheet-open by `ChatRepository.resolveVisionCapable()` AND a local PDF via `PageImageSource.pageCountIfLocal`; a 1-based page-pick stepper dialog; `AskViewModel.runVisionPreset(instruction, pageNumber)` converts 1→0-based **once**, renders the page network-free, re-checks vision at action time (an on-device engine becoming ready mid-sheet → `ask_error_no_vision`, not a silent figure-less degrade), and re-enters the same `prepare → confirm → stream` path with the attachment via the shared `handlePrepared`; null image → `ask_error_page_image`. Network-free local-PDF glob only; a text/no-attachment turn stays byte-identical so the redaction goldens are untouched).
- **R3b.2** ✅ — **follow-up suggestion chips:** heuristic (Quick/Standard/on-device) via a pure `FollowUpHeuristics` generator, and model-generated (cloud-Max) via a `FOLLOWUPS::` user-turn suffix that a shared pure `extractFollowUps` strips (final-line-only) from both the displayed and persisted body, falling back to heuristics. Chips re-enter `ask()` on tap, render only on the latest settled turn. No golden impact (Standard untouched).
- **R4** ✅ — **save/export/share (text/Markdown):** share an answer or the whole conversation as Markdown via the OS share sheet (pure `ConversationMarkdown` serializer + `ACTION_SEND`/`EXTRA_TEXT`; no FileProvider). User-initiated, OS-sheet only, chat content never routed through the importable backup schema. Wired in all three Ask hosts.
- **R4.2** ✅ — rendered **image** export (answer/diagram → PNG), shipped as **P-Share PS.4**. A `FileProvider` (`${applicationId}.fileprovider`, `cache/ask_exports/`) + `shareImage` (ACTION_SEND `image/png` + EXTRA_STREAM + **`ClipData`** so the chooser preview gets the grant). Capture is **host-driven** (`RichImageExporter`): the answer renders in a sheet-occluded WebView at `alpha=0.004` (non-zero so the compositor produces frames and the async KaTeX/Mermaid render actually completes), the Kotlin host polls **host-initiated `evaluateJavascript`** for "fonts loaded + every Mermaid `<svg>` present" (immune to the off-screen renderer's page-timer/rAF throttling), measures the **max element bottom** (a Mermaid `<svg>` overflows its body box), then software-layer `draw()` → PNG. The earlier plan's **page-JS render-signal** (`arxiver://rendered` on `fonts.ready`+rAF) and the `createPrintDocumentAdapter`→PDF idea were both **discarded on device evidence** (K17): an off-screen WebView produces no frames, so rAF/page-timers don't fire, and the print-adapter fires on `onPageFinished` (before async JS renders). Wired into the PS.3 per-bubble overflow for rich answers. A PDF-for-tall-answers path + a native knowledge-map PNG reuse the same FileProvider — documented fast-follows.
- **R4-TTS** — read-aloud an answer (aligns with the planned P3 `TextToSpeech`).
...
## 10. App-deterministic artifacts (P-Atlas)

The keystone insight of Phase P-Atlas: **the app draws the structure; the AI only narrates it.** Research is unanimous that LLM-emitted diagrams fail ~15–20% of syntax checks (MermaidSeqBench, StructEval), while a diagram composed programmatically from structured data is **valid by construction**. Arxiver already owns the data (embedding cosines + citation edges, Phase 4.9 `relations`), so it can render the relationships itself — on **every** tier (cloud, on-device, or none) and **offline**.

- **Builder (`RelationGraphBuilder`, `:core:search`, pure):** `RelationGraph` (nodes + similarity/citation edges) → a valid ` ```mermaid ` flowchart. Mobile-graph rules: a hard node cap (~12) to avoid node-soup on a phone; **theme-safe node shapes** (center = hexagon, in-library = rectangle, external = rounded — no hardcoded colors, so Mermaid's light/dark `themeVariables` drive the palette, and a statically-inserted graph adapts when the theme toggles); deduped edges; labels escaped for `securityLevel:'strict'` (`"` → `#quot;`, `#` → `#35;`, `$` dropped, whitespace collapsed, length-capped). Returns null when there's nothing to draw, so the caller can show empty-state copy. Fully unit-tested (golden Mermaid, adversarial titles, node cap, empty cases).
- **Feeder (`RelationGraphRepository`, `:app`, network-free):** reuses the `relations` primitives — `embeddingDao.neighborsFor` (precomputed `related_papers`) or a live `VectorIndex.topK` scan for a freshly-embedded paper, plus `citationDao.observeReferences/observeCitations`. Returns a typed `GraphResult` (`Ready` / `NotEmbedded` / `NoRelations`) so each sparse-data cause gets specific empty-state copy. Exposed to the VM via the narrow `RelationGraphSource` seam (keeps the ViewModel DAO-free + unit-testable).
- **Insertion (no provider call):** `ChatRepository.insertArtifactTurn(scope, sessionId, question, content)` persists a complete `STATUS_COMPLETE` user + assistant pair (the assistant carries the Mermaid) and **bypasses `prepare()`/`stream()`/the privacy preview entirely** — nothing leaves the device, so the redaction goldens are untouched. `AskViewModel.runGraphArtifact` mirrors it into the live UI. The session's `providerId` is recorded as on-device (the artifact is computed locally).
- **Surface:** a `map_relations` `AskPreset` (`artifact = true`, PAPER scope) routed to `runGraphArtifact` instead of the model path. Renders through the unchanged `RichContent.has` + `RichBlockWebView`. Collection-scope graphs + a library knowledge-map are PA.5.
- **Device (`VERIFICATION.md`):** the `Map relationships` chip renders a valid graph for a paper with neighbors, the right empty-state copy for a fresh/un-embedded paper, **with no AI provider configured** and **in airplane mode** (proving model-independence).
